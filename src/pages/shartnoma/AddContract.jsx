...
import { 
  ...
  X, Briefcase, Users, ShoppingCart, Calendar, CreditCard, Save
} from 'lucide-react';
...
const AddContract = () => {
  ...
  const [contractData, setContractData] = useState({
    ...
    coBorrowers: [],         
    staffId: '',             
    items: [],               // Tanlangan tovarlar
    duration: 12,            // Muddat (oy)
    prepayment: 0,           // Oldindan to'lov
    paymentDay: 1,           // Har oyning qaysi sanasi
    hasBonus: true,
    bonusPercent: 0,
    note: ''
  });
  ...
  // UI States
  const [productTab, setProductTab] = useState('catalog'); // 'catalog' | 'cart'
  ...
  // --- HISOBLASHLAR ---
  const grandTotal = contractData.items.reduce((sum, item) => sum + (item.salePrice * item.qty), 0);
  ...
  const generateSchedule = () => {
      ...
      for(let i = 1; i <= contractData.duration; i++) {
          ...
          // Agar tanlangan sana joriy oydagi kunlardan ko'p bo'lsa (masalan 31-fevral), oxirgi kunini oladi
  ...
  const updateItemQty = (id, newQty) => {
      ...
      setContractData(prev => ({
          ...prev,
          items: prev.items.map(item => item.id === id ? { ...item, qty: newQty } : item)
      }));
  };

  const removeItem = (id) => {
      setContractData(prev => ({ ...prev, items: prev.items.filter(i => i.id !== id) }));
  };

  // --- QIDIRUV KOMPONENTI (Ichki) ---
  ...
  const submitContract = () => {
      console.log("Jo'natilmoqda:", contractData);
      toast.success("Shartnoma muvaffaqiyatli saqlandi! (Backend hali ulanmagan)");
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="max-w-7xl mx-auto px-4 mt-6 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        {/* --- CHAP TOMON (SIDEBAR - MA'LUMOTLAR) --- */}
        <div className="lg:col-span-4 space-y-4 lg:sticky lg:top-24">
            ...
            {contractData.mainCustomer && (
                <div className="bg-white p-5 rounded-2xl shadow-sm border border-blue-100 relative overflow-hidden group animate-in fade-in slide-in-from-left-4">
                    ...
                    <div className="relative z-10">
                        ...
                        <h3 className="font-black text-gray-800 text-lg leading-tight uppercase mb-4">{contractData.mainCustomer.lastName} <br/> {contractData.mainCustomer.firstName} {contractData.mainCustomer.middleName}</h3>
                        <div className="grid grid-cols-2 gap-4 text-xs">
                            <div>
                                <p className="text-gray-400 mb-0.5">Pasport</p>
                                <p className="font-bold text-gray-700">{contractData.mainCustomer.document?.series} {contractData.mainCustomer.document?.number}</p>
                            </div>
                            <div>
                                <p className="text-gray-400 mb-0.5">JSHSHIR</p>
                                <p className="font-mono font-bold text-gray-700">{contractData.mainCustomer.pinfl}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {/* Qarz oluvchilar kartochkasi */}
            {contractData.coBorrowers.map((co, idx) => (
                <div key={co.id} className="bg-white p-5 rounded-2xl shadow-sm border border-purple-100 relative overflow-hidden animate-in fade-in">
                    <div className="flex justify-between items-start mb-3">
                    ...
                    <h3 className="font-bold text-gray-800 text-sm uppercase mb-3">{co.lastName} {co.firstName}</h3>
                    <div className="flex justify-between text-xs border-t border-gray-50 pt-3">
                        <span className="text-gray-500 font-mono">Pass: {co.document?.series} {co.document?.number}</span>
                        <span className="text-gray-500 font-mono">JSHSHIR: {co.pinfl}</span>
                    </div>
                </div>
            ))}

            {/* Savat xulosasi (Agar tovar tanlangan bo'lsa) */}
        ...
        <div className="lg:col-span-8">
                ...
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 animate-in slide-in-from-right-8">
                    ...
                    <div className="space-y-6">
                        <div>
                            ...
                            {!contractData.mainCustomer ? (
                                <SearchableSelect 
                                    placeholder="JSHSHIR, pasport yoki ism bo'yicha qidiring..." 
                        ...
                        <div className="pt-6 border-t border-gray-100">
                            ...
                            <SearchableSelect 
                                placeholder="Qo'shimcha javobgarlarni qo'shish..." 
                                onSelect={addCoBorrower} 
                                excludeIds={[contractData.mainCustomer?.id, ...contractData.coBorrowers.map(c => c.id)].filter(Boolean)} 
                            />
                        </div>
                        ...
                        <div className="pt-6 border-t border-gray-100">
                            ...
                            <div className="relative">
                                ...
                                <select 
                                    className={`w-full pl-12 pr-4 py-3 border rounded-xl outline-none font-medium appearance-none transition-colors ${contractData.staffId ? 'border-blue-500 bg-blue-50 text-blue-800' : 'border-gray-200 bg-gray-50 text-gray-700 focus:bg-white focus:border-blue-500'}`} 
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[600px] animate-in slide-in-from-right-8">
                    <div className="flex border-b border-gray-100 bg-gray-50">
                        ...
                        <button onClick={() => setProductTab('cart')} className={`flex-1 py-4 text-sm font-bold transition-colors relative ${productTab === 'cart' ? 'bg-white text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}>
                            Tanlangan tovarlar 
                            {contractData.items.length > 0 && <span className="absolute top-1/2 -translate-y-1/2 right-10 bg-red-500 text-white text-[10px] px-2 py-0.5 rounded-full">{contractData.items.length}</span>}
                        </button>
                    </div>
                    ...
                    {productTab === 'catalog' && (
                        <div className="p-6 flex flex-col flex-1 overflow-hidden">
                            <div className="relative mb-4">
                                ...
                                <input type="text" placeholder="Tovar nomi yoki kodini qidiring..." value={productSearch} onChange={(e) => setProductSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-sm"/>
                            </div>
                                ...
                                <table className="w-full text-left text-sm">
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                        {filteredProducts.map(p => {
                                            ...
                                            return (
                                                <tr key={p.id} className={`hover:bg-blue-50 transition-colors ${isAdded ? 'opacity-50 bg-gray-50' : ''}`}>
                                                    ...
                                                    <td className="p-3 text-right font-bold text-gray-800">{p.salePrice.toLocaleString()}</td>
                                                    <td className="p-3 text-center">
                                                        <button disabled={isAdded || p.quantity <= 0} onClick={() => addProductToCart(p)} className="p-2 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white disabled:bg-gray-100 disabled:text-gray-300 transition-colors">
                        ...
                        <div className="p-6 flex flex-col flex-1 overflow-hidden bg-gray-50/50">
                            {contractData.items.length === 0 ? (
                                <div className="flex-1 flex flex-col items-center justify-center text-gray-400">
                                    ...
                                    <p>Savat bo'sh. Katalogdan tovar tanlang.</p>
                                </div>
                            ) : (
                                <div className="flex-1 overflow-y-auto custom-scrollbar border bg-white border-gray-100 rounded-xl shadow-sm">
                                    <table className="w-full text-left text-sm">
                                        <thead className="bg-gray-50 sticky top-0 text-[10px] text-gray-500 uppercase tracking-wider">
                                            <tr>
                                                <th className="p-3">Nomi</th>
                                        ...
                                        <tbody className="divide-y divide-gray-100">
                                            {contractData.items.map(item => (
                                                <tr key={item.id}>
                                                    ...
                                                    <td className="p-3 text-center">
                                                        <input type="number" min="1" max={item.quantity} value={item.qty} onChange={(e) => updateItemQty(item.id, Number(e.target.value))} className="w-full p-2 border rounded-lg text-center outline-blue-500 font-bold bg-gray-50"/>
                                                    </td>
                                                    <td className="p-3 text-right font-medium text-gray-600">{item.salePrice.toLocaleString()}</td>
                                                    <td className="p-3 text-right font-black text-blue-600">{(item.salePrice * item.qty).toLocaleString()}</td>
                                                    <td className="p-3 text-center">
                                                        <button onClick={() => removeItem(item.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg"><Trash2 size={16}/></button>
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
            ...
            {step === 3 && (
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 animate-in slide-in-from-right-8">
                    <h2 className="text-xl font-black text-gray-800 mb-6">To'lov grafigi va muddat</h2>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {/* Chap tomon: Sozlamalar */}
                        <div className="space-y-6">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-3">Necha oyga berilmoqda?</label>
                                <div className="grid grid-cols-4 gap-3">
                                    {[3, 6, 9, 12, 15, 18, 24].map(m => (
                                        <button 
                                            key={m} 
                                            onClick={() => setContractData(prev => ({...prev, duration: m}))}
                                            className={`py-3 rounded-xl font-black text-lg transition-all ${contractData.duration === m ? 'bg-blue-600 text-white shadow-md shadow-blue-200 scale-105' : 'bg-gray-50 text-gray-600 hover:bg-gray-100 border border-gray-200'}`}
                                        >
                                            {m}
                                        </button>
                                    ))}
                                    <div className="col-span-4 mt-2">
                                        <input type="number" placeholder="Yoki o'zingiz kiriting (oy)" value={contractData.duration} onChange={(e) => setContractData(prev => ({...prev, duration: Number(e.target.value)}))} className="w-full p-3 border border-gray-200 rounded-xl outline-blue-500 text-sm font-medium bg-gray-50 focus:bg-white"/>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="p-5 bg-gray-50 rounded-xl border border-gray-200 space-y-4">
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Boshlang'ich to'lov (UZS)</label>
                                    <input type="number" value={contractData.prepayment} onChange={(e) => setContractData(prev => ({...prev, prepayment: Number(e.target.value)}))} className="w-full p-3 border rounded-xl outline-blue-500 font-bold text-lg text-emerald-600" />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">To'lov sanasi (Har oyning)</label>
                                    <div className="flex items-center gap-2">
                                        <input type="number" min="1" max="31" value={contractData.paymentDay} onChange={(e) => setContractData(prev => ({...prev, paymentDay: Number(e.target.value)}))} className="w-20 p-3 border rounded-xl outline-blue-500 font-bold text-center" />
                                        <span className="text-sm font-medium text-gray-600">- sanasida</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-gray-50 p-1 rounded-xl border border-gray-200 flex flex-col h-full max-h-[400px]">
                            <div className="p-4 border-b border-gray-200 bg-white rounded-t-xl flex justify-between items-center">
                                <span className="font-bold text-gray-700">Grafik (Preview)</span>
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
                                <table className="w-full text-sm text-left">
                                    <thead className="text-[10px] text-gray-400 uppercase sticky top-0 bg-gray-50">
                                        <tr><th className="py-2 px-3">Oy</th><th className="py-2 px-3">Sana</th><th className="py-2 px-3 text-right">Summa</th></tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-100 font-medium">
                                        {generateSchedule().map(row => (
                                            <tr key={row.month} className="hover:bg-white">
                                                <td className="py-2 px-3 text-gray-500">{row.month}</td>
                                                <td className="py-2 px-3 text-gray-800">{row.date}</td>
                                                <td className="py-2 px-3 text-right text-gray-800">{Math.round(row.amount).toLocaleString()}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
            ...
            {/* QADAM 4: YAKUNIY TASDIQ VA IZOH */}
            {step === 4 && (
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 animate-in slide-in-from-right-8">
                    <h2 className="text-xl font-black text-gray-800 mb-6 flex items-center gap-2"><CheckCircle className="text-emerald-500"/> Yakuniy sozlamalar</h2>
                    
                    <div className="space-y-6 max-w-xl">
                        <div className="flex gap-6 p-4 bg-gray-50 rounded-xl border border-gray-200">
                            <label className="flex items-center gap-2 cursor-pointer font-bold text-gray-700">
                                <input type="radio" name="bonus" checked={contractData.hasBonus} onChange={() => setContractData(prev => ({...prev, hasBonus: true}))} className="w-5 h-5 text-blue-600"/> Bonus hisoblansin
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer font-bold text-gray-700">
                                <input type="radio" name="bonus" checked={!contractData.hasBonus} onChange={() => setContractData(prev => ({...prev, hasBonus: false}))} className="w-5 h-5 text-blue-600"/> Bonus berilmasin
                            </label>
                        </div>

                        {contractData.hasBonus && (
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Bonus miqdori (%)</label>
                                <input type="number" value={contractData.bonusPercent} onChange={(e) => setContractData(prev => ({...prev, bonusPercent: Number(e.target.value)}))} className="w-1/3 p-3 border rounded-xl outline-blue-500 font-bold" />
                            </div>
                        )}

                        <div>
                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Shartnoma bo'yicha maxsus izoh</label>
                            <textarea value={contractData.note} onChange={(e) => setContractData(prev => ({...prev, note: e.target.value}))} rows="4" className="w-full p-4 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50 focus:bg-white transition-colors" placeholder="Faqat ushbu shartnoma uchun kerak bo'ladigan izohlar (ixtiyoriy)..."></textarea>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="mt-6 flex justify-end gap-4">
                {step > 1 && (
                    <button onClick={() => setStep(step - 1)} className="px-8 py-3 bg-white border border-gray-200 text-gray-700 rounded-xl font-bold hover:bg-gray-50 transition-colors">
                        Orqaga
                    </button>
                )}
                <button 
                    onClick={handleNext} 
                    className={`px-10 py-3 rounded-xl font-black flex items-center gap-2 transition-all shadow-lg ${step === 4 ? 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-emerald-200' : 'bg-blue-600 hover:bg-blue-700 text-white shadow-blue-200'}`}
                >
                    {step === 4 ? <><Save size={20}/> Saqlash</> : <>Davom etish <ChevronRight size={20}/></>}
                </button>
            </div>
