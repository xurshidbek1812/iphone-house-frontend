...
const ProfileSettings = () => {
  ...
  useEffect(() => {
    const fetchMyProfile = async () => {
      try {
        ...
        const me = await parseJsonSafe(res);

        if (!res.ok || !me) {
          throw new Error(me?.error || me?.message || "Profilni yuklab bo'lmadi");
        }

        const parts = String(me.fullName || '').trim().split(' ').filter(Boolean);
        ...
        console.error(err);
        toast.error(err.message || 'Profilni yuklashda xatolik!');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSaveInfo = async () => {
    if (!userData.firstName.trim() || !userData.lastName.trim()) {
      return toast.error('Ism va familiyani kiriting!');
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.error || data?.message || "Saqlab bo'lmadi");
      }
  ...
  const handleChangeLogin = async () => {
    if (!securityForm.newValue || securityForm.newValue.length < 4) {
      return toast.error("Yangi login kamida 4 ta belgidan iborat bo'lishi kerak!");
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.error || data?.message || "Bu login band yoki xato yuz berdi!");
      }
  ...
  const handleChangePassword = async () => {
    ...
    if (securityForm.newValue.length < 4) {
      return toast.error('Yangi parol juda qisqa!');
    }

    if (securityForm.newValue !== securityForm.confirmValue) {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center mb-6">
        ...
        {isLoading && <Loader2 className="animate-spin text-blue-600" size={22} />}
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="grid grid-cols-3 gap-6">
            <div>
              ...
              {isEditingInfo ? (
                <input
                  type="text"
                  className="w-full p-2.5 border rounded-lg outline-blue-500 bg-gray-50 focus:bg-white"
                  value={userData.firstName}
                  onChange={(e) => setUserData({ ...userData, firstName: e.target.value })}
                />
              ) : (
                <div className="text-lg font-medium text-gray-800 border-b border-transparent py-2">
                  {userData.firstName || '-'}
                </div>
              )}
            </div>
          ...
          {isEditingInfo && (
            <div className="flex justify-end gap-3 mt-4 animate-in fade-in">
              ...
              <button
                ...
                className="px-4 py-2 text-white bg-blue-600 rounded-lg hover:bg-blue-700 font-bold"
              >
                Saqlash
              </button>
            </div>
        ...
        <div className="p-8">
          <h3 className="font-bold text-gray-400 uppercase text-xs tracking-wider mb-6">
          ...
          <div className="grid grid-cols-2 gap-8">
            <div className="bg-blue-50 p-5 rounded-xl border border-blue-100">
              ...
              <button
                onClick={() => setIsChangingLogin(true)}
                className="mt-4 w-full py-2 bg-white text-blue-600 border border-blue-200 rounded-lg font-bold hover:bg-blue-600 hover:text-white transition-colors text-sm"
              >
            ...
            <div className="bg-gray-50 p-5 rounded-xl border border-gray-200">
              ...
              <button
                ...
                className="mt-4 w-full py-2 bg-white text-gray-700 border border-gray-300 rounded-lg font-bold hover:bg-gray-700 hover:text-white transition-colors text-sm"
              >
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 animate-in zoom-in-95">
            ...
            <div className="space-y-4">
              <div className="relative">
                <label className="block text-sm font-bold text-gray-700 mb-1">Joriy parol</label>
                <input
                  type={showCurrentPass ? 'text' : 'password'}
                  className="w-full p-3 pr-10 border border-gray-300 rounded-xl outline-blue-500 bg-white"
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full p-3 pr-10 border rounded-xl outline-green-500 bg-white"
                  placeholder="Yangi parol..."
                  value={securityForm.newValue}
                  onChange={(e) => setSecurityForm({ ...securityForm, newValue: e.target.value })}
                />
                <button
                  onClick={() => setShowNewPass(!showNewPass)}
                  className="absolute right-3 top-9 text-gray-400 hover:text-green-600"
                >
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full p-3 pr-10 border rounded-xl outline-green-500 bg-white"
                  placeholder="Yangi parolni qaytadan yozing"
                  value={securityForm.confirmValue}
                  onChange={(e) =>
                    setSecurityForm({ ...securityForm, confirmValue: e.target.value })
                  }
                />
                <button
            ...
            <div className="mt-8 flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-3 bg-blue-600 rounded-xl font-bold text-white shadow-lg hover:bg-blue-700"
              >
                Saqlash
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="text"
                    className="w-full pl-8 p-3 border rounded-xl outline-blue-500 bg-white"
                    placeholder="Yangi login..."
                    value={securityForm.newValue}
                    onChange={(e) =>
                      setSecurityForm({ ...securityForm, newValue: e.target.value })
                    }
                  />
                </div>
              </div>
