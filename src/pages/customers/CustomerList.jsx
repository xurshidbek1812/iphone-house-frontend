...
const CustomerList = () => {
  ...
  const fetchCustomers = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/customers?${params.toString()}`, {
        headers: {
      ...
      if (res.status === 403) {
        console.error('403 response:', data);
        toast.error("Bu bo'limga ruxsat yo'q yoki token yaroqsiz.");
        setCustomers([]);
        return;
      }

      if (!res.ok) {
        console.error('Customers fetch xato:', data);
        toast.error(data?.error || 'Mijozlarni yuklashda xatolik yuz berdi');
        setCustomers([]);
        return;
      }
      ...
      if (Array.isArray(data)) {
        ...
        console.error('Customers array emas:', data);
        setCustomers([]);
      }
    } catch (error) {
      console.error('Customer fetch xatolik:', error);
  ...
  const executeDelete = async () => {
    try {
      const res = await fetch(`${API_URL}/api/customers/${deleteModal.id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`
        }
                ...
                customers.map((customer) => (
                  <tr key={customer.id} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="p-4 text-gray-600">
                      {customer.dob ? new Date(customer.dob).toLocaleDateString('ru-RU') : '-'}
                    </td>

                    <td className="p-4 text-gray-600 font-mono">{customer.pinfl || '-'}</td>

                    <td className="p-4 text-gray-600 font-mono">
                      {customer.phones?.[0]?.phone || customer.phone || '-'}
                    </td>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[1000] p-4">
          <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl p-6 animate-in zoom-in-95">
            ...
            <p className="text-center text-gray-500 text-sm mb-6">
              ...
              Bu mijozning barcha ma'lumotlari butunlay o'chib ketadi.
            </p>
