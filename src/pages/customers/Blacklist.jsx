...
const Blacklist = () => {
  ...
  useEffect(() => {
    const fetchBlacklist = async () => {
      try {
        const res = await fetch(`https://iphone-house-api.onrender.com/api/customers`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        const data = await res.json();
        // Faqat qora ro'yxatdagilarni ajratib olamiz
        setCustomers(data.filter(c => c.isBlacklisted === true));
      } catch (error) {
        ...
        <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-100 text-sm">
                    {loading ? (
                        <tr><td colSpan="5" className="p-6 text-center text-gray-500">Yuklanmoqda...</td></tr>
                    ) : customers.length === 0 ? (
                        <tr>
                        ...
                        customers.map((c) => (
                            <tr key={c.id} className="hover:bg-red-50/50 transition-colors group">
                                ...
                                <td className="p-4 font-bold text-gray-800 uppercase">{c.lastName} {c.firstName} {c.middleName}</td>
                                <td className="p-4 text-gray-600">{new Date(c.dob).toLocaleDateString('ru-RU')}</td>
